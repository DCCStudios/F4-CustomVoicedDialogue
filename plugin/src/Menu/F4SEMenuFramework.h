...
// CustomVoicedDialogue patch: resolved lazily instead of at static-init.
// This plugin loads alphabetically BEFORE F4SEMenuFramework.dll, so a
// load-time GetModuleHandle would always come back null.  All framework
// calls happen at kPostLoad or later, when the module is present.
...
namespace F4SEMenuFramework {
    ...
    namespace Model {
        namespace Internal {
            ...
            T GetFunction(LPCSTR name) {
                if (!menuFramework) {
                    menuFramework = GetModuleHandleW(L"F4SEMenuFramework");
                }
                if (!menuFramework) {
                    return nullptr;
                }
    ...
    // ─── Plugin Hotkey API ─────────────────────────────────────────────────────
    // Lightweight hotkey registration: the framework handles WndProc dispatch,
    // INI persistence, and conflict detection. Each hotkey is identified by a
    // unique string id (e.g. "MyMod.ToggleOverlay"). If the user has already
    // rebound this id in the framework INI, the persisted binding takes precedence
    // over the default scan code you pass at registration time.
    namespace Hotkeys {
        ...
        // Register a hotkey. Returns a handle for later Unregister().
        ...
        // Unregister a previously registered hotkey by handle.
        ...
        // Get the current scan code for a hotkey id (reflects user rebinding).
        ...
        // a warning notification is shown on-screen automatically.
        ...
        // Check if a scan code would conflict with another registered hotkey
        // (excluding excludeId). Call before SetBinding to proactively warn users.
