...
public sealed class InworldProvider : ITtsProvider
{
    ...
    public async Task<string> AutoTagAsync(string text, string voiceType, bool isPlayer, ProviderSettings settings, CancellationToken cancellationToken, string voicePath = "") =>
        (await AutoTagDetailedAsync(text, voiceType, isPlayer, settings, cancellationToken, voicePath)).Text;

    public async Task<AutoTagResult> AutoTagDetailedAsync(string text, string voiceType, bool isPlayer, ProviderSettings settings, CancellationToken cancellationToken, string voicePath = "")
    {
        ...
        try
        {
            ...
            var take = 1 + VoiceMapping.VoiceMapper.Fnv1a(voicePath.Length > 0 ? voicePath : text) % 7;
            var messages = new List<object>
            {
                new Dictionary<string, string> { ["role"] = "system", ["content"] = TaggerSystemPrompt },
            ...
            if (originalWords.Length > 0 && SpokenWords(tagged) == originalWords)
            {
