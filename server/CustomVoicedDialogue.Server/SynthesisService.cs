...
public sealed class SynthesisService
{
    ...
    public string PlayerVoiceFingerprint()
    {
        ...
        // hand-lexicon-only stage, "ipa2" added rule-derived pronunciations, "ipa3" the accent-true r symbols, "ipa4" the Australian vowel overhaul, "ipa5" its KT-Speech corrections, "ipa7" its weakened-glide PRICE, "ipa8" merged IPA spans, "ipa9" the Grimes yeah stretch, "ipa10" its general vowel-holding, "ipa11" stress-tripled emphasis.
        // Bumping it regenerates accented audio after a mechanism change.
        var accentPart = accent.IsNeutral ? "" : $"|ipa11:{accent.Id}|{_config.AccentImperfection}";
        return Fingerprint(
    ...
    public string NpcVoiceFingerprint()
    {
        ...
        var accentPart = accents.Length == 0 ? "" : $"|ipa11:{accents}|{_config.AccentImperfection}";
        return Fingerprint(
